...
if (!fs.existsSync(accuracyImgDir)) {
  console.error("report/accuracy not found. Run screenshot first.");
  process.exit(1);
}
const stgFiles = fs.readdirSync(accuracyImgDir)
  .filter((f) => f.endsWith("-stg.png"))
  .sort();

if (stgFiles.length === 0) {
  console.error("No screenshot files found in report/accuracy/.");
  process.exit(1);
}
...
for (const stgFile of stgFiles) {
  ...
  const stgBuf = fs.readFileSync(path.join(accuracyImgDir, stgFile));
  const prdBuf = fs.readFileSync(path.join(accuracyImgDir, prdFile));
  ...
  fs.writeFileSync(path.join(accuracyImgDir, `${baseName}-diff.png`), diffPngBuf);

  const detected = mismatched > 0;
  const status = detected ? "DIFF" : "PASS";

  console.log(
    `  ${baseName.padEnd(8)} | ${status.padEnd(4)} | ${String(mismatched).padStart(6)} px`,
  );
...
// サマリー計算
const totalCount = accuracyResults.length;
const diffCount = accuracyResults.filter((r) => r.detected).length;
const passCount = accuracyResults.filter((r) => !r.detected).length;

console.log(`\nDiff: ${diffCount}, Pass: ${passCount}, Total: ${totalCount}`);

// HTML レポート生成

const tableRows = accuracyResults
  .map(
    (r, i) => `
...
<body>
<h1>Accuracy Report \u2014 STG vs PRD</h1>
...
<table>
<thead>
  <tr><th>Page</th><th>Status</th><th>Pixels</th><th>Preview</th></tr>
</thead>
<tbody>
${tableRows}
</tbody>
...
fs.writeFileSync(path.join(accuracyImgDir, "report.html"), html);

console.log(`\nSaved: ${accuracyImgDir}/report.html`);
console.log(`Saved: ${accuracyImgDir}/ (${accuracyResults.length} diff images)`);
